...
## Issues Encountered
- **Mock verification compiler version:** `auto_detect_solc` resolves MockERC20's `^0.8.20` pragma to 0.8.28 (the highest installed), not 0.8.24. Verification needed `--compiler-version 0.8.28` to match the deployed bytecode. `VerifyPhase1.sh` pins this. The 4 mocks (D-23 gate), plus factory + quoterV2, are source-verified on Mantlescan. SwapRouter/NPM source-verification is best-effort and still pending (their code is present on-chain, codesize>0, which is what the checkpoint requires); they can be re-submitted later without affecting downstream plans.
...
- **Pending:** Task 3 human-verify checkpoint — the human must confirm the verified mock source pages render and the pool quotes are mainnet-like before downstream plans trust these addresses. This SUMMARY does NOT mark the plan fully complete.
